...
class PredictelligenceEngine:
    ...
    def _warm_up(self):
        for _ in range(3):
            self.pipeline.run("SW1A1AA", 285000, 285000, "investor")
        self._warm_up_done = True

    def analyse(self, postcode, current_valuation, comparable_average, user_type):
        state = self.pipeline.run(postcode, current_valuation, comparable_average, user_type)
        return {
